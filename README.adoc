...
Cette commande à lancer 14 tests unitaires (14 méthodes de test) situés dans le dossier `tests`. Les tests vérifient le comportement de certains objets du projet (instances des classes `CardTest`, `CardGameTest` et `GameTest`)
...
TIP: **TODO PROF** : première évaluation sur la capacité de l'étudiant à étudier : questions autour du code présenté et premiers concepts gravitant autour des tests unitiares.
...
Dans la fenêtre `Terminal` en bas, vous devriez pouvoir lancer la commande `./bin/phpunit` et obtenir ceci :
...
1 . Dans le terminal du projet, lancer la commande `./bin/phpunit  tests/Core/CardTest`. Le résultat devrait être celui-ci :
...
* Livraison par mail d'une version *pdf* de votre README au plus tard *jeudi 16 septembre 2022 23h59*.
